...
fn ensure_schema(connection: &mut Connection) -> StoreResult<bool> {
    ...
    // Version 1: legacy schema with color/created/modified/text_direction columns.
    // Version 2: renamed columns (colour, created_at, modified_at, direction) + title.
    // Each migration step is idempotent — safe to re-run if interrupted mid-flight.
    if current_version < 2 {
        ...
        if !columns.contains("created_at") {
            ...
            if columns.contains("created") {
                transaction.execute(
                    "UPDATE notes SET created_at = CAST(created AS INTEGER)",
                    [],
                )?;
            }
        }
        ...
        if !columns.contains("direction") {
            ...
            if columns.contains("text_direction") {
                transaction.execute(
                    "UPDATE notes SET direction = text_direction",
                    [],
                )?;
            }
        }
