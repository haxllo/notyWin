...
use windows_sys::Win32::{
    ...
    UI::{
        ...
        WindowsAndMessaging::{
            ...
            DefWindowProcW, DestroyAcceleratorTable, DestroyMenu, DrawMenuBar, EnableMenuItem,
            GetCursorPos, GetMenu, GetMenuItemInfoW, InsertMenuW, PostMessageW, PostQuitMessage,
            RemoveMenu, SendMessageW, SetForegroundWindow, SetMenu, SetMenuItemInfoW, ShowWindow,
...
type DefSubclassProcFn = unsafe extern "system" fn(HWND, u32, WPARAM, LPARAM) -> LRESULT;
...
impl SubclassApi {
    unsafe fn load() -> Option<Self> {
        let module = unsafe { LoadLibraryW(encode_wide("comctl32.dll").as_ptr()) };
        if module.is_null() {
            return None;
        }
        let def_subclass_proc = unsafe {
            std::mem::transmute(GetProcAddress(module, b"DefSubclassProc\0".as_ptr())?)
        };
        let remove_window_subclass = unsafe {
            std::mem::transmute(GetProcAddress(module, b"RemoveWindowSubclass\0".as_ptr())?)
        };
        let set_window_subclass = unsafe {
            std::mem::transmute(GetProcAddress(module, b"SetWindowSubclass\0".as_ptr())?)
        };
        Some(Self {
            def_subclass_proc,
...
static SUBCLASS_API: Lazy<Option<SubclassApi>> = Lazy::new(|| unsafe { SubclassApi::load() });

unsafe fn set_window_subclass(
    hwnd: HWND,
    subclass_proc: SUBCLASSPROC,
    subclass_id: usize,
    reference_data: usize,
) -> BOOL {
    SUBCLASS_API.as_ref().map_or(FALSE, |api| unsafe {
        (api.set_window_subclass)(hwnd, subclass_proc, subclass_id, reference_data)
    })
}

unsafe fn remove_window_subclass(hwnd: HWND, subclass_proc: SUBCLASSPROC, subclass_id: usize) -> BOOL {
    SUBCLASS_API.as_ref().map_or(FALSE, |api| unsafe {
        (api.remove_window_subclass)(hwnd, subclass_proc, subclass_id)
    })
}

unsafe fn def_subclass_proc(hwnd: HWND, message: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    SUBCLASS_API.as_ref().map_or_else(
        || unsafe { DefWindowProcW(hwnd, message, wparam, lparam) },
        |api| unsafe { (api.def_subclass_proc)(hwnd, message, wparam, lparam) },
    )
}
...
impl Menu {
    ...
    unsafe fn remove_from_hwnd(hwnd: isize) {
        ...
        // SAFETY: HWND validity is upheld by caller
        remove_window_subclass(hwnd, Some(menu_subclass_proc), MENU_SUBCLASS_ID);
    ...
    pub unsafe fn detach_menu_subclass_from_hwnd(&self, hwnd: isize) {
        // SAFETY: HWND validity is upheld by caller
        remove_window_subclass(hwnd as _, Some(menu_subclass_proc), MENU_SUBCLASS_ID);
    }
...
impl MenuChild {
    ...
    pub unsafe fn detach_menu_subclass_from_hwnd(&self, hwnd: isize) {
        // SAFETY: HWND validity is upheld by caller
        remove_window_subclass(hwnd as _, Some(menu_subclass_proc), SUBMENU_SUBCLASS_ID);
    }
}
    ...
    match msg {
        ...
        WM_COMMAND => {
            ...
            if let Some(item) = item {
                ...
                def_subclass_proc(hwnd as _, msg, wparam, lparam)
            }
        }
        ...
        WM_NCACTIVATE | WM_NCPAINT => {
            ...
            let res = def_subclass_proc(hwnd as _, msg, wparam, lparam);
        ...
        _ => def_subclass_proc(hwnd as _, msg, wparam, lparam),
    }
}
