...
pub struct Controller {
    ...
    hover_poll_timer: Timer,
    deck_hovered: bool,
    hovered_display_id: Option<u64>,
    idle_timer_armed: bool,
    hover_generation: u64,
    hover_flags: Vec<std::rc::Rc<std::sync::atomic::AtomicBool>>,
...
impl Controller {
    pub fn bind(ui: &Rc<NotyWindow>, state: AppState) -> Rc<RefCell<Self>> {
        ...
        let controller = Rc::new_cyclic(|weak| {
            RefCell::new(Self {
                ...
                hover_poll_timer: Timer::default(),
                deck_hovered: false,
        ...
        controller
            .borrow_mut()
            .hover_flags
            .push(std::rc::Rc::new(std::sync::atomic::AtomicBool::new(false)));
    ...
    pub fn attach_window(controller: &Rc<RefCell<Self>>, ui: &Rc<NotyWindow>, display_id: u64) {
        Self::install_close_handler(ui);
        let hover_flag = std::rc::Rc::new(std::sync::atomic::AtomicBool::new(false));
        {
            let mut ctrl = controller.borrow_mut();
            ctrl.windows.push((ui.clone(), display_id));
            ctrl.hover_flags.push(hover_flag.clone());
        }
        platform::set_hover_flag(&ui.window(), std::rc::Rc::as_ptr(&hover_flag));
        let weak_controller = Rc::downgrade(controller);
        let weak_ui = ui.as_weak();
    ...
    fn handle_deck_hover(&mut self, inside: bool, display_id: Option<u64>) {
        ...
        if inside {
            ...
            if !expanded {
                let was_rest = self.state.deck_state == DeckState::Rest;
                ...
                if was_rest && self.state.deck_state == DeckState::Fan {
                    // The pill TouchArea is destroyed when transitioning to
                    // fan. Slint does not fire has-hover(false) on destruction,
                    // so deck_hovered stays true and collapse never triggers.
                    // Reset the flag and schedule a short timer; tabs will
                    // fire true to re-arm if the pointer is over one.
    ...
    pub fn refresh(controller: &Rc<RefCell<Self>>) {
        ...
        for ((ui, display_id), hover_flag) in windows.into_iter().zip(hover_flags) {
            ...
            let fullscreen_suppressed = false;
...
fn deck_hit_test_mode(
    ...
    let mut regions = Vec::with_capacity(shown_count + 5);

    // Pill column: full-height strip where the pill lives, so the mouse never
    // loses hover when transitioning from pill to fan.
    let pill_width = physical(12.0 * deck_scale * display_scale);
    let pill_x = if left_edge { 0 } else { frame.width as i32 - pill_width as i32 };
    ...
    // Tab column: full-height strip covering all tabs and controls, so hover
    // persists when the pointer is anywhere in the tab region.
    let tab_column_left = if left_edge { 0 } else { item.x };
    let tab_column_right = if left_edge {
    ...
    regions.push(HitTestRect {
        x: tab_column_left,
        y: 0,
        width: (tab_column_right - tab_column_left) as u32,
        height: frame.height,
    });
    ...
    // Slint owns tab hit targets; avoid rejecting transformed tab clicks.
    let _ = regions;
    HitTestMode::Full
}
